...
To define within-trainloop function grabs, put your specified function within **data/mlp_grabs.py**, make sure \*\*kwargs is taken as an argument, and specify that the function should be used by with the *other_model_grabs* argument. If your function uses the model, or if there is anything specific to your function such as a fixed index, use the *other_model_gram* argument ("call_with_model" for using the model, or else it defaults to grabbing weight matrices). An example is shown below:
